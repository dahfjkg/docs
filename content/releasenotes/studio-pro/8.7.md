...
* When you add a second [language](/howto/collaboration-requirements-management/translate-your-app-content) in your Mendix application and try to switch between languages using the [shortcut keys](/refguide/menus#language), an error might occur.
	* Workaround: Select the desired language using the menu on the right side of the status bar, or select the language via **Language** > **Language Settings** once. After that, you can use the shortcut keys to switch between the languages.
* SQL Server integrated security does not work and causes the following error: "This driver is not configured for integrated authentication."
	* Workaround: New DLLs are needed to work with the updated JDBC driver and should be placed inside the *runtime/lib/x86* and *runtime/lib/x64* directories of the Studio Pro 8.7 install location. You can download the correct DLLs from [8.2.0 Stable Release](https://github.com/microsoft/mssql-jdbc/releases/tag/v8.2.0), where you should select *mssql-jdbc_auth.zip*.
